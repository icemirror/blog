...
node程序中为客户端设置响应(response)头部时, 需要设置```Content-Type```字段, 需要根据返回的数据类型, 设置与之对应的类型.
...
在已知固定响应内容类型时可以设置固定的```Content-Type```, 但在实际编程过程中, 我们需要编写一个根据资源后缀动态判断资源类型的方法, 动态的去设置 ```Content-Type```值, 这里我们需要了解MIME.

<kbd>MIME</kbd> (Multipurpose Internet Mail Extensions) 是描述消息内容类型的因特网标准。

设置响应头```Content-Type```, 可以有效的帮助浏览器正确的解析资源(虽然目前现代浏览器已经具备了推断并正确解析资源的能力, 但服务端能够正确响应```Content-Type```信息也是有必要的). MIME标准是可枚举的, 因此在服务端常见解决思路是: 获取响应资源的后缀, 根据后缀查找对应的```Content-Type```.
...
module.exports = (filePath) => {
    let ext = path.extname(filePath) // 获取文件后缀
        .split('.')
        .pop()
        .toLowerCase(); // 取文件后缀名
    if (!ext) {
    ...
    return mimeTypes[ext] || mimeTypes['txt']; // 默认为文本类型
}
/*app.js*********************************************/
...
const server = http.createServer((req, res) => {
  const filePath = path.join(root, req.url); // 请求资源路径
